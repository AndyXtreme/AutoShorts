...
| Variable | UI (Settings →) | Default | Effect |
| :--- | :--- | :--- | :--- |
| `SUBTITLE_FONT_SIZE` | Caption Layout → Caption font size | `0` | `0` keeps the template's own size (24 for `hype`). Captions render at twice this value in pixels, so the ceiling is set by the **longest single word**: it cannot wrap, and once wider than the frame it is cut off at the edges. Around `30` stays safe for long German compounds. Outline and padding scale with the size |
